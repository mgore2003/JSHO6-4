...
function showAll(selectList) {
	let options = document.querySelectorAll("select#make option");
...
function filterSelect(selectList, category) {
	let options = document.querySelectorAll("select#model option");
	let optionLength = options.length;
	for (let i = 0; i < options.length; i++) {
...
selectVehicle.addEventListener("click", function(){
	vehicle.innerHTML = make.text + model.text + trim.text;
});
}